...
# Replace Lembaga charts (BPD and PKK)
lembaga_charts_html = """
          <div class="text-center mt-5" data-aos="fade-up">
...
# I need to carefully replace the old HTML.
import re
...
content = re.sub(
    r'<div class="text-center mt-5"[^>]*>[\s\S]*?id="collapseBaganLembaga"[^>]*>[\s\S]*?id="chart_bpd"[^>]*></div>\s*</div>\s*<div class="col-12 mt-5">[\s\S]*?id="chart_pkk"[^>]*></div>\s*</div>\s*</div>\s*</div>\s*</div>',
    lembaga_charts_html,
    content
)

# Remove the JS charts logic at the bottom
content = re.sub(
    r'<!-- GOOGLE ORG CHART JS -->[\s\S]*?</script>',
    '',
    content
)
# Remove the custom org chart CSS
content = re.sub(
    r'/\* Styling khusus untuk Google Org Chart Node \*/[\s\S]*?}\n',
    '',
    content
)
...
with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated profil.blade.php for static images")
