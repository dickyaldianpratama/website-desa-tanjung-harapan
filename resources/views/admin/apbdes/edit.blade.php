    ...
    <div class="col-md-8 mx-auto">
        <div class="card-admin mb-4">
            <div class="card-header">
                <i class="bi bi-pencil-square me-2 text-gold"></i>Edit Data APBDes
            </div>
            <div class="card-body p-4">
                <form action="{{ route('admin.apbdes.update', $apbde->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    
                    <div class="mb-3">
                        <label for="tahun" class="form-label fw-bold">Tahun Anggaran</label>
                        <input type="number" class="form-control" id="tahun" name="tahun" value="{{ old('tahun', $apbde->tahun) }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="uraian" class="form-label fw-bold">Uraian / Deskripsi</label>
                        <input type="text" class="form-control" id="uraian" name="uraian" value="{{ old('uraian', $apbde->uraian) }}" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="jenis" name="jenis" required>
                            ...
                            <option value="belanja" {{ old('jenis', $apbde->jenis) == 'belanja' ? 'selected' : '' }}>Belanja</option>
                        </select>
                    </div>

                    <div class="mb-3">
                        <label for="anggaran" class="form-label fw-bold">Anggaran (Rp)</label>
                        <input type="text" class="form-control input-rupiah" id="anggaran" name="anggaran" value="{{ old('anggaran', $apbde->anggaran) }}" required>
                    </div>

                    <div class="mb-4">
                        <label for="realisasi" class="form-label fw-bold">Realisasi (Rp)</label>
                        <input type="text" class="form-control input-rupiah" id="realisasi" name="realisasi" value="{{ old('realisasi', $apbde->realisasi) }}" required>
                    </div>
