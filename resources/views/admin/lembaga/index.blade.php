...
<div class="page-header">
    <div class="d-flex align-items-center gap-3">
        ...
        <div>
            <h4 class="fw-bold text-dark mb-1">lembaga Desa</h4>
    ...
    <div class="header-actions d-flex align-items-center gap-3 flex-wrap">
        <form action="{{ route('admin.lembaga.index') }}" method="GET" class="m-0 flex-grow-1 d-flex gap-2">
            <select name="tipe" class="form-select border-0 bg-white" style="width: auto; box-shadow: inset 0 2px 4px rgba(0,0,0,0.01);" onchange="this.form.submit()">
                <option value="">Semua Tipe</option>
                <option value="BPD" {{ request('tipe') == 'BPD' ? 'selected' : '' }}>BPD</option>
                <option value="PKK" {{ request('tipe') == 'PKK' ? 'selected' : '' }}>PKK</option>
            </select>
        ...
        <a href="{{ route('admin.lembaga.create') }}" class="btn btn-primary btn-tambah px-4 py-2" style="background: linear-gradient(135deg, #c9963a 0%, #a67828 100%); border: none; border-radius: 12px; font-weight: 600; box-shadow: 0 4px 15px rgba(201, 150, 58, 0.3); transition: transform 0.2s;">
...
<div class="mb-4">
    <div class="alert alert-light border d-inline-block rounded-pill px-4 py-2 shadow-sm">
        ...
        <a href="{{ route('admin.lembaga.index') }}" class="ms-3 text-danger text-decoration-none"><i class="bi bi-x-circle me-1"></i>Hapus Filter</a>
    </div>
</div>
@endif
...
<div class="lembaga-grid">
    @forelse($lembagas as $p)
        <div class="id-card">
            <div class="id-card-header">
                <div class="badge-urutan">#{{ $p->urutan }} - {{ $p->tipe }}</div>
            </div>
            
            <div class="id-card-photo-wrapper">
                @if($p->foto)
                    <img src="{{ Storage::disk('s3')->url('images/lembaga/' . $p->foto) }}" alt="{{ $p->nama }}" class="id-card-photo">
                @else
            ...
            <div class="id-card-footer">
                <a href="{{ route('admin.lembaga.edit', $p->id) }}" class="btn-act btn-edit" title="Edit Data">
                    <i class="bi bi-pencil-square"></i>
                </a>
                <form action="{{ route('admin.lembaga.destroy', $p->id) }}" method="POST" class="m-0 delete-form">
                    @csrf
                    @method('DELETE')
    ...
    @empty
        <div class="empty-state">
            ...
            <h4 class="fw-bold text-dark mb-2">Belum Ada lembaga Desa</h4>
            <p class="text-muted mb-4">Tambahkan data Kepala Desa beserta lembaga jajarannya di sini.</p>
            <a href="{{ route('admin.lembaga.create') }}" class="btn px-4 py-2" style="background: #c9963a; color: white; border-radius: 10px; font-weight: 600;">
...
<script>
    document.querySelectorAll('.btn-delete-action').forEach(button => {
        button.addEventListener('click', function(e) {
            Swal.fire({
                title: 'Hapus Data?',
                text: "Data pejabat/lembaga desa yang dihapus tidak bisa dikembalikan lagi.",
                icon: 'warning',
